document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateQR(answer) {
        ...
        qrValue = `https://dormantone.github.io/foodforthought/formPage.html?username=${encodeURIComponent(username)}&question=${question}&answer=${encodeURIComponent(answer)}`;

        var qr = new QRious({
            element: document.getElementById('qrCodeCanvas'),
            size: 200,
            value: qrValue // Use the form page URL as the QR code value
        });
        ...
        firebase.firestore().collection('qrcodes').add({
            ...
            answer: answer,
            url: qrValue, // Store the URL instead of the data URL
            timestamp: new Date()
        }).then(docRef => {
            docId = docRef.id;
        });
    ...
    function sendEmail() {
        ...
        var body = encodeURIComponent('Access your QR code here: ') + 
                   encodeURIComponent(qrValue); // Use qrValue which contains the form page URL
